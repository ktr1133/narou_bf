...
function writerRankSql($c, $t, $gan, $uf){
    //$c = $_GET['time_span'];
    //$t = $_GET['time_span'];
    //$gan = $_GET['gan_num'];
    //$uf = $_GET['frequency'];
      ...
      if($cate['cate']==='mark'){
          if(!empty($t)){
            if($t==='monthly'){
              ...
              $group_by_calc = 'AVG(`mark`.`'.$order_by_basic.'`)';
            }else if($t==='half'){
            ...
            if(!empty($t)){
                if($t==='monthly'){
                  $order_by_basic = 'mean_monthly';
                  $group_by_calc = 'AVG(`calc`.`'.$order_by_basic.'`)';
                }else if($t==='half'){
                  $order_by_basic = 'mean_half';
                  $group_by_calc = 'AVG(`calc`.`'.$order_by_basic.'`)';
                }else if($t==='all'){
          ...
          if(!empty($t)){
            if($t==='monthly'){
              ...
              $group_by_calc = 'SUM(`point`.`'.$order_by_basic.'`)';
            }else if($t==='half'){
        ...
        if(!empty($t)){
            if($t==='monthly'){
              $order_by_basic = 'sum_monthly';
              $group_by_calc = 'SUM(`unique`.`'.$order_by_basic.'`)';
            }else if($t==='half'){
              $order_by_basic = 'sum_half';
              $group_by_calc = 'SUM(`unique`.`'.$order_by_basic.'`)';
            }else if($t==='all'){
    ...
    if($cate['cate'] !=='ma'){
      $sql_basic_temp = "SELECT `ma`.`ncode`,`ma`.`title`, `ma`.`writer`, `ma`.`general_all_no`,
             COUNT(`ma`.`writer`) AS `count`, ".$group_by_calc." AS `ave`
             FROM `ma`, `".$requiredTable."` WHERE ".$requiredLink.$filter." GROUP BY `ma`.`writer`";
      $basicTbl = '( '.$sql_basic_temp.' ) AS `b`';
    }else{
      $sql_basic_temp = "SELECT `ma`.`ncode`,`ma`.`title`, `ma`.`writer`, `ma`.`general_all_no`,
             COUNT(`ma`.`writer`) AS `count` FROM `ma` GROUP BY `ma`.`writer`";
      $basicTbl = '( '.$sql_basic_temp.' ) AS `b`';
    }
    ...
    if(!empty($where)){
        ...
        if(isset($where_uf)){
          if($cate['cate'] ==='ma'){
            $sql = 'SELECT `b`.`ncode`, `b`.`title`, `b`.`writer`, `b`.`count`,'.$tbl_uf.' FROM '.$basicTbl.' , '.$tableSQL.' WHERE '.$linkSQL.' AND '.$whereSQL.$where_uf.' ORDER BY `count` '.$order.' LIMIT 50;';
          }else{
            $sql = 'SELECT `b`.`ncode`, `b`.`title`, `b`.`writer`, `b`.`count`, `b`.`ave`,'.$tbl_uf.' FROM '.$basicTbl.' , '.$tableSQL.' WHERE '.$linkSQL.' AND '.$whereSQL.$where_uf.' ORDER BY `b`.`ave` '.$order.' LIMIT 50;';
          }
        }else{
          if($cate['cate'] ==='ma'){
            $sql = 'SELECT `b`.`ncode`, `b`.`title`, `b`.`writer`, `b`.`count` FROM '.$basicTbl.' , '.$tableSQL.' WHERE '.$linkSQL.' AND '.$whereSQL.' ORDER BY `count` '.$order.' LIMIT 50;';
          }else{
            $sql = 'SELECT `b`.`ncode`, `b`.`title`, `b`.`writer`, `b`.`count`, `b`.`ave` FROM '.$basicTbl.' , '.$tableSQL.' WHERE '.$linkSQL.' AND '.$whereSQL.' ORDER BY `b`.`ave` '.$order.' LIMIT 50;';
          }
        }
    }else{
      if($cate['cate'] ==='ma'){
        $sql = 'SELECT `b`.`ncode`, `b`.`title`, `b`.`writer`, `b`.`count` FROM '.$basicTbl.' ORDER BY `count` '.$order.' LIMIT 50;';
      }else{
        $sql = 'SELECT `b`.`ncode`, `b`.`title`, `b`.`writer`, `b`.`count`, `b`.`ave` FROM '.$basicTbl.' ORDER BY `b`.`ave` '.$order.' LIMIT 50;';
      }
    }
    return $sql;
}
